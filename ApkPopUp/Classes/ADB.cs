...
namespace ApkPopUp
{
    class ADB
    {
        ...
        private static void InstallAdb()
        {
            ...
            if (!File.Exists(appDataDir + "//adb.exe"))
                File.WriteAllBytes(appDataDir + "//adb.exe", Properties.Resources.adb);
            if (!File.Exists(appDataDir + "//AdbWinApi.dll"))
                File.WriteAllBytes(appDataDir + "//AdbWinApi.dll", Properties.Resources.AdbWinApi);
            
            ADBPath = appDataDir + "//adb.exe";
        }
        #endregion
